...
def login():
    ...
    if form.validate_on_submit():
        ...
        if usuario and bcrypt_var.check_password_hash(usuario[0].senha, form.senha.data):
            ...
            app.logger.log(SUCCESS, f"{current_user.usuario} | {request.access_route[-1]} Usuário logado{" (sessão permanente)" if form.lembrar_me.data else ""}")
            return redirect(url_for("menu"))

        else:

            form.erros['login'] = 'Nome de usuário ou senha incorretos'
            app.logger.error(f"{request.access_route[-1]} Nome de usuário ou senha incorretos")
...
def logout():
    ...
    if current_user.is_authenticated:
        app.logger.log(SUCCESS, f"{current_user.usuario} | {request.access_route[-1]} Usuário saiu")
        logout_user()
